...
if __name__ == '__main__':
    ...
    input_video = f"/home/jianing/research/3d-motion/dataset/{args.file}/r-{args.file}.mp4"
    output_dir = f"/home/jianing/research/3d-motion/Video-Depth-Anything/output/r-{args.file}"
    ...
    processed_video_path = os.path.join(output_dir, args.file+'_src.mp4')
    depth_vis_path = os.path.join(output_dir, args.file+'_vis.mp4')
    save_video(frames, processed_video_path, fps=fps)
    save_video(depths, depth_vis_path, fps=fps, is_depths=True, grayscale=args.grayscale)

    if args.save_npz:
        depth_npz_path = os.path.join(output_dir, args.file+'_depths.npz')
        np.savez_compressed(depth_npz_path, depths=depths)
    if args.save_exr:
        depth_exr_dir = os.path.join(output_dir, args.file+'_depths_exr')
